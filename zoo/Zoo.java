package ZooManagementSystem.zoo;

import ZooManagementSystem.animals.*;
import ZooManagementSystem.app.Main;
import ZooManagementSystem.common.*;
import ZooManagementSystem.data.AnimalManagment;
...
import ZooManagementSystem.people.Worker;
...
import static ZooManagementSystem.common.Gender.*;
...
public class Zoo {
	...
	private static List<Penguin> penguins = new ArrayList<Penguin>(max_size);
	...
	private static int[] numOfFishColours= new int[Colour.values().length+1];
	private static int numberOfLions;
	private static int numberOfPenguins;
	private static int numberOfFishes;
	private static int numberOfLynxes;
	private static int numberOfTigers;
	...
	private static Visitor[] visitors = new Visitor[max_size];
	private static List<WorkersData> pass_pairs;

	private static AnimalManagment animalManagment = new AnimalManagment();
	...
	public Zoo()   {
		...
		animalIDs=numberOfDogs+numberOfPenguins+numberOfLions+numberOfFishes+numberOfLynxes+numberOfLynxes;
		...
		// Changing the initalizition of the database to make sure it would return the same datebase
		String adminUsername = getConfigValue("zoo.admin.username", "ZOO_ADMIN_USERNAME", "Admin");
		String adminPassword = getConfigValue("zoo.admin.password", "ZOO_ADMIN_PASSWORD", "");
		WorkersData admin= WorkersData.getInstance(adminUsername, adminPassword);
		pass_pairs = Arrays.asList(admin);
		...
		Worker worker1 = new Worker(1234,
				...
				512345678 ,
				adminUsername);
		visitors[0] = new Visitor(1,"Sample","Visitor","15/01/2001",512345679);
	...
	public void fetchData() {
		...
		try (Connection connection = DatabaseConnection.getConnection()) {
			...
			String query = "SELECT species, COUNT(*) as count FROM Animal GROUP BY species";
            ...
            while (resultSet.next()) {
                ...
                switch (species.toLowerCase()) {
                    case "lion":
                        ...
                        break;
                    case "tiger":
                        numberOfTigers = count;
                        break;
                    case "penguin":
                        numberOfPenguins = count;
                        break;
                }
            }
            resultSet.close();
            ...
            System.out.println("Database is unavailable; using the in-memory zoo data.");
        }
    }
	...
	public static Visitor getVisitor(int index) {
	...
	public static List<WorkersData> getWorkerCredentials() {
		return pass_pairs;
	}

	public static void incrementFishColourCount(int colourIndex) {
		numOfFishColours[colourIndex]++;
	}
	...
	public static List<Penguin> addPenguin(Penguin newPenguin) {
		...
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs,"Penguin",newPenguin.getName(),newPenguin.getAge(),"Healthy","Arctic",1);
		int sortby = Penguin.getLastSortWayused();
		if(sortby==1) {
	...
	public static void AddNewFish(int type_f, int age_f, double length_f, Colour[] singularFishColours,Pattern Pattern_F){
		...
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs++,"Fish",new_fish.getPattern().toString(), new_fish.getAge(),"Healthy","Sea",1);
	...
	public static AquariumFish[] addFish(int num) {
		if(num+numberOfFishes > fishes.length) {
			...
			for (int i = numberOfFishes; i < numberOfFishes+num; i++) {
				...
				if(typeOfFish == 2) {
					...
					animalIDs+=(int)(Math.random() * 9999 + 1);
					animalManagment.addAnimal(animalIDs++,"Fish",concatArray[i].getPattern().toString(), concatArray[i].getAge(),"Healthy","Sea",1);
					continue;
				}
				if(typeOfFish == 3) {
		...
		for (int i = numberOfFishes; i < numberOfFishes+num; i++) {
			...
			if(typeOfFish == 2) {
				...
				animalIDs+=(int)(Math.random() * 9999 + 1);
				animalManagment.addAnimal(animalIDs++,"Fish",fishes[i].getPattern().toString(), fishes[i].getAge(),"Healthy","Sea",1);
				continue;
			}
			if(typeOfFish == 3) {
	...
	public static Lion[] addLion(Lion newLion) {
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs++,"Lion", newLion.getName(), newLion.getAge(), "Healthy","Savannah",1);
	...
	public static Tiger[] addTiger(Tiger newTiger){
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs++,"Tiger", newTiger.getName(), newTiger.getAge(), "Healthy","Savannah",1);
	...
	public static Dog[] addDog(Dog newDog){
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs++,"Dog", newDog.getName(), newDog.getAge(), "Healthy","Savannah",1);
	...
	public static Lynx[] addLynx(Lynx newLynx){
		animalIDs+=(int)(Math.random() * 9999 + 1);
		animalManagment.addAnimal(animalIDs++,"Lynx", newLynx.getName(), newLynx.getAge(), "Healthy","Savannah",1);
	...
	public static String MostPopularFishColour() {
		if (numberOfFishes == 0) {
			return "There are no fish colours to report yet.";
		}
		int index = -1;
		int secondIndex = -1;
		int max = 0;
		...
		for(int i = 0; i < Colour.values().length; i++) {
			if(numOfFishColours[i] > max) {
				...
				secondIndex = index;
				index = i;
			} else if(numOfFishColours[i] > secondMax) {
				secondMax = numOfFishColours[i];
				secondIndex = i;
			}
		}
		if (index < 0) {
		...
		if (secondIndex < 0) {
			return "The most common colour in the aquarium is:\n" + Colour.values()[index];
		}
	...
	public static boolean ConnectionToVMSCheck(String usr, String pass){
		return Worker.ConnectToVMS(usr, pass);
	}
